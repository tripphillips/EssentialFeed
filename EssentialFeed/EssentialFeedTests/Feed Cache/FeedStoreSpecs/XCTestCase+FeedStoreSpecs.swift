...
extension FeedStoreSpecs where Self: XCTestCase {
    ...
    func insert(_ cache: (feed: [LocalFeedImage], timestamp: Date),
        ...
        sut.insert(cache.feed, timestamp: cache.timestamp) { receivedInsertionErrorResult in
            if case let Result.failure(error) = receivedInsertionErrorResult { insertionError = error }
            exp.fulfill()
        }
    ...
    func deleteCache(from sut: FeedStore,
        ...
        sut.deleteCachedFeed { retrievedDeletionErrorResult in
            if case let Result.failure(error) = retrievedDeletionErrorResult { deletionError = error }
            exp.fulfill()
        }
        wait(for: [exp], timeout: 1.0)
    ...
    func expect(_ sut: FeedStore,
        ...
        sut.retrieve { retrievedResult in
            ...
            case let (.success(.some(expectedResult)), .success(.some(retrievedResult))):
                XCTAssertEqual(expectedResult.feed, retrievedResult.feed, file: file, line: line)
                XCTAssertEqual(expectedResult.timestamp, retrievedResult.timestamp, file: file, line: line)
            default:
                XCTFail("Expected to retrieve \(expectedResult), \(retrievedResult) instead", file: file, line: line)
            }
            exp.fulfill()
        }
        wait(for: [exp], timeout: 1.0)
